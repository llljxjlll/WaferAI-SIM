...
## 6. V4-d：大小、速率、latency 与多跳扫描（完成）

- 消息大小 `F=1/7/8/9/128`：wire 始终各一个 REQUEST/ACK/DATA 代表消息；逻辑包数
  与 `S(F)` 无损保留，包含原 V3 depth=8 两侧边界。
- 有理数速率：`1、2/3、1/2、1/4`，分别覆盖 port/link 限速与非瓶颈变化；
  `R=min(1,port,link)` 和 ceil 全部与独立 oracle 精确一致。
- latency `L=0/1/7/20`，一跳和 3×1 两跳同时扫描：fixed 分别为 `3L` 与 `6L`；
  两跳仍只计一次 bulk service，wire crossing 与 re-pin 随 `H` 成比例。

验证从 V4 **4/4→9/9**。除账本相等外还检查真实完成时间：

- 同一 workload 改速率：`ΔT=CYCLE·ΔS(F)`；
- 改 latency：一跳 `ΔT=CYCLE·3L`、两跳 `ΔT=CYCLE·6L`；
- 实测一跳 `L=0/1/7/20` 为 `262/268/304/382 ns`，两跳为
  `316/328/400/556 ns`。
...
- `run_v0_exit.py` 原输出“全版本准入门”已过时；现明确命名为“历史 V0–V3 兼容门”，
  并指向当前 superset `run_v4_exit.py`。文件名保留以兼容旧 CI。
- Behavioral 的 NoC cut=`1/1` 在 C++ 与独立 Python oracle 两处增加 `TODO(V5)`：
  多 lane/striping 必须使用真实共享 NoC cut 的有理数速率，禁止简单使用
  `lane_count * min(single-lane rate)` 高估吞吐。

验证：Python 语法检查、oracle **8/8**、历史兼容门三项全部 PASS，NoC 冻结值不变。
这是 `d2d-v4-baseline` 之后的维护提交，冻结 tag 不移动。
