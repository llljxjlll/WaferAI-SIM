...
- **V2-d ✔（延迟标定与活性验收）**：
  ...
  - **NoC 与 D2D 分离**（可归因，而非笼统「变慢」）：`T(H,L) = T_noc(H) + 3*H*L*CYCLE`。
    对比 1 跳与 2 跳：`T1 = 278/284/320/398`、`T2 = 332/344/416/572`，得
    **多出一跳的 NoC 开销 = `T2(0)-T1(0)` = 54 ns（与 L 无关）**，而
    `(T2(L)-T1(L)) - 54 = 0/6/42/120` **精确等于 `3*L*CYCLE`**——即该跳的 D2D 部分严格正比于 L。
  - **watchdog / 活性**：V2 用功能性无限 FIFO，合法多跳模式必须始终推进到完成；任一次运行
    命中超时哨兵（124）即判失败，把「永久挂起」变成可见失败而非卡住。16 次扫描运行 0 超时。
...
- **当前验证 / V2 完成**：纯函数/路由自测 **241/241**、Link SystemC 自测 **18/18**、
  D2D runner **58/58**、NoC 冻结值 **14781/29109、14833/45441**。
