...
**版本标识（冻结的最后一步，待授权）**：当前工作树含大量未提交/未跟踪的 V0 改动，`HEAD`（7940afa）**无法**
复现当前 165/165 + 23/23 + NoC 门全过的状态。正式冻结还需：① 将 V0 变更形成明确 commit；② 在此处记录完整
commit SHA；③ 打 tag `d2d-v0-baseline`；④ 在**干净工作树**上重跑 `run_v0_exit.py` 全绿。**提交/tag 属版本
管理动作，待用户明确授权后执行**（届时回填 SHA）。
