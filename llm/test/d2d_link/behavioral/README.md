...
- 无争用 `F=128`：cycle 的真实稳态 goodput 对 `R=1,1/2,1/4` 均在 oracle 1% 内；
  Behavioral 的 `S(F)` 与相同 oracle 精确相等。
- mixed Local+D2D shared/disjoint：cycle 产生 `11/0` 次 die0 NoC stall，跨 die flow
  相差 36 cycle；Behavioral stall 均为 0，逻辑服务账本相同，总时长差≤1 cycle。
- 两条 Behavioral D2D flow 共用同一 link：各自独立计 service，并行完成时间与单 flow
  相同，明确证明不维护跨-flow资源占用。

因此两个后端的适用边界是：

- Behavioral：快速、可解释的无争用估算；
- cycle：有限资源、背压、NoC/link 争用与死锁安全。
