#!/usr/bin/env python3
"""Distributed DRAMSys real SRAM/HBM production transport regression."""
...
def main() -> int:
    command = [
        ...
        "--hardware-config", "../llm/test/sram/hardware_dramsys.json",
    ...
    try:
        ...
        print("[SRAM DRAMSYS] FAIL: workload timed out")
        return 1
    if proc.returncode != 0:
        print(proc.stdout)
        print(f"[SRAM DRAMSYS] FAIL: npusim returned {proc.returncode}")
        return 1
    records = {
    ...
    if records.keys() != expected:
        print(f"[SRAM DRAMSYS] FAIL: marker set is {set(records)}")
        return 1
    ...
    if len(checksums) != 1 or next(iter(checksums)) <= 0 or any(
        ...
        print(f"[SRAM DRAMSYS] FAIL: data records are {records}")
        return 1
    for engine in ("lsu", "dte"):
        if records[(engine, "double_buffer")][3] >= records[(engine, "blocking")][3]:
            print(f"[SRAM DRAMSYS] FAIL: {engine} double buffer did not improve time")
            return 1
    print(
        "[SRAM DRAMSYS] PASS: LSU/DTE real payload round-trip checksum",
        checksums.pop(),
    )
    print("[SRAM DRAMSYS] PASS: distributed DRAMSys backend preserves overlap")
    return 0
