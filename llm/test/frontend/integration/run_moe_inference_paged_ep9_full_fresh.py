"""Two independent fixed-model nine-Die low-HBM MoE offload runs."""
...
_COMPILE = Path(__file__).with_name("run_moe_inference_paged_ep9_compile_canary.py")
_RELINK = _ROOT / "llm/frontend/wafer_frontend/passes/moe_inference_paged_compile_sequence_ep9.py"
_SIDECAR = _ROOT / "llm/frontend/wafer_frontend/passes/moe_inference_paged_runtime_ep9.py"
_SCHEMA = "moe-ep9-low-hbm-two-full-fresh-v1"
...
def _sidecar_module():
    for name, path in (
        ("llm.frontend.wafer_frontend.passes.moe_inference_paged_compile_sequence_ep9", _RELINK),
        ("llm.frontend.wafer_frontend.passes.moe_inference_paged_runtime_ep9", _SIDECAR),
    ):
    ...
    return sys.modules[
        "llm.frontend.wafer_frontend.passes.moe_inference_paged_runtime_ep9"
    ]
...
def _source_contract(rows: int, columns: int):
    ...
    external = MemoryTierCapacity.create(
        ...
        base_address=0, capacity_bytes=12288, alignment_bytes=16,
    )
    try:
...
def _hardware(rows: int, columns: int) -> dict:
    ...
    if (len(physical["hbm_stacks"]) != 9
            or any(item["backend"] != "behavioral"
                   for item in physical["hbm_stacks"])):
        raise ValueError("physical nine-Die behavioral HBM layout changed")
...
def _native_observation(fresh: Path, contract: dict, rows: int, columns: int) -> dict:
    ...
    if sidecar != contract or len(sidecar["events"]) != 215:
        raise ValueError("physical EP9 sidecar differs from source-derived contract")
    receipt = _json(compiled / "compile_canary_receipt.json")
    if (receipt["status"] != "compile_finalizer_program_io_only"
            or receipt["mesh"] != f"{rows}x{columns}" or receipt["ep"] != 9):
    ...
    for step, segment in enumerate(receipt["segments"]):
        if (step != segment["step"] or segment["runtime_core_ids"] != [0, 4, 8, 12, 16, 20, 24, 28, 32]
                or segment["program_io_initializations"] != 109
                or segment["program_io_probes"] != 1):
            raise ValueError("EP9 compiled segment/source geometry drifted")
        for role in ("source", "paged"):
            for suffix, name in (("manifest_sha256", "linked.json"),
                ...
                if _sha(path) != segment[role][suffix]:
                    raise ValueError("EP9 production artifact SHA drifted")
        if _sha(compiled / f"segment_{step}.program_io.json") != segment["program_io_sha256"]:
            raise ValueError("EP9 ProgramIO SHA drifted")
    ...
    if len(dma) != 215 or {int(x[0]) for x in dma} != set(range(215)):
        raise ValueError("215 actual source-bound EP9 DMA events absent")
    ...
    for index, event in enumerate(sidecar["events"]):
        ...
        if (int(row[1]) != event["segment_index"]
                ...
                or row[10:] != ("1", "1")):
            raise ValueError(f"physical EP9 DMA gate {index} differs from linked StateABI")
    drain = re.findall(
    ...
    if drain != [("215", "12", "215", "215", "16360", "10944",
                  "10944", "16360", "0", "0", "0", "1")]:
        raise ValueError("physical EP9 external/HBM page traffic did not close")
    ...
    if (len(admission) != 1 or int(admission[0][0]) <= 0
        ...
        raise ValueError("nine EP cores did not queue behind fixed two-outstanding link")
    ...
    if (len(memory) != 9 or {item[0] for item in memory} != {0, 4, 8, 12, 16, 20, 24, 28, 32}
            or any(issued <= 0 or issued != completed or lsu or dte
                   for _, issued, completed, lsu, dte in memory)):
        raise ValueError("physical nine-core LSU/DTE memory did not close")
    ...
    return {
        ...
        "ep": 9, "mesh": f"{rows}x{columns}", "hbm_capacity_bytes_per_die": 1024,
        "external_capacity_bytes": 12288,
        "dma_events": len(dma), "external_read_bytes": 16360,
        "external_write_bytes": 10944,
...
def run(args: argparse.Namespace) -> None:
    if args.mesh != "3x3":
        raise ValueError("only audited nine-Die meshes are supported")
    ...
    for index in range(2):
        ...
        if not fresh.exists():
            ...
            if done.returncode:
                raise RuntimeError(f"independent EP9 compile failed Fresh{index}")
        ...
        contract = builder.build_moe_inference_paged_runtime_ep9(
        ...
        if args.resume:
            if (_json(contract_path) != contract
                ...
                raise ValueError("source-derived EP9 sidecar or hardware drifted")
        else:
            ...
            if done.returncode:
                raise RuntimeError(f"physical EP9 native Fresh{index} failed exit={done.returncode}; see {native_path}")
        ...
        if args.resume:
            if _json(evidence_path) != json.loads(json.dumps(item)):
                raise ValueError("physical EP9 Fresh receipt drifted")
        else:
            evidence_path.write_text(json.dumps(item, indent=2, sort_keys=True))
            print(f"MoE EP9 low-HBM PASS full_fresh_{index}", flush=True)
    ...
    if stable(observed[0]) != stable(observed[1]):
        raise ValueError("two independent EP9 full materializations disagree")
    case = {
    ...
    if args.resume:
        if _json(case_path) != json.loads(json.dumps(case)):
            raise ValueError("full EP9 case receipt drifted")
    else:
        case_path.write_text(json.dumps(case, indent=2, sort_keys=True))
    if _binding(args, source_root) != binding:
        raise ValueError("EP9 source/native tool drifted during full Fresh")
    print("MoE EP9 low-HBM VERIFIED two independent full materializations", flush=True)
...
def main() -> None:
    ...
    parser.add_argument("--mesh", choices=("3x3",), default="3x3")
