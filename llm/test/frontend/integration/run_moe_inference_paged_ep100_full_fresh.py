...
_SIDECAR = _ROOT / "llm/frontend/wafer_frontend/passes/moe_inference_paged_runtime_ep100.py"
_SCHEMA = "moe-ep100-low-hbm-two-full-fresh-v1"
...
def _sidecar_module():
    for name, path in (
        ...
        ("llm.frontend.wafer_frontend.passes.moe_inference_paged_runtime_ep100", _SIDECAR),
    ):
    ...
    return sys.modules[
        "llm.frontend.wafer_frontend.passes.moe_inference_paged_runtime_ep100"
    ]
...
def _source_contract(rows: int, columns: int):
    ...
    try:
        ...
        raise ValueError("same-model resident fits 2048B/Die unexpectedly")
...
def _native_observation(fresh: Path, contract: dict, rows: int, columns: int) -> dict:
    ...
    if (receipt["status"] != "compile_finalizer_program_io_only"
            or receipt["mesh"] != f"{rows}x{columns}" or receipt["ep"] != 100):
    ...
    return {
        ...
        "ep": 100, "mesh": f"{rows}x{columns}", "hbm_capacity_bytes_per_die": 2048,
...
def run(args: argparse.Namespace) -> None:
    ...
    for index in range(2):
        ...
        contract = builder.build_moe_inference_paged_runtime_ep100(
