"""Two independent full materializations of the low-HBM full Dense inference pair."""
...
def audit_full_fresh(directory: Path) -> dict[str, object]:
    report = json.loads((directory / "dense-inference-paged-runtime-evidence.json").read_text(encoding="utf-8"))
    ...
    if (hardware.get("x"), hardware.get("y")) != (2, 2) or hardware.get("die") != {"x": 1, "y": 1}:
    ...
    if report.get("kv_page_bytes") != [0, 512, 640, 768] or report.get("kv_versions") != [0, 1, 2, 3]:
        raise ValueError("external KV versions or real bytes are incomplete")
    if report.get("external_dma_events") != 65 or report.get("external_kv_probes") != 12 or report.get("functional") is not False:
        raise ValueError("real external transfer/probe contract or timing mode disagrees")
    ...
    for native in (0, 1):
        ...
        if active_cores != {0}:
            raise ValueError("actual NpuSim low-HBM inference ran outside physical Die0/core0")
        if "[DENSE_INFERENCE_PAGED_DMA_DRAIN] events=65" not in stdout:
...
def run(args: argparse.Namespace) -> None:
    ...
    for index in (0, 1):
        ...
        command = (
            sys.executable, "-m", "llm.test.frontend.integration.run_dense_inference_paged_offload_runtime_canary",
    ...
    print("Dense full inference low-HBM external offload PASS two independent full materializations", flush=True)
