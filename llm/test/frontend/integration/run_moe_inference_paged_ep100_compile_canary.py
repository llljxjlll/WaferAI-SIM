"""Compile, relink, finalize and build ProgramIO for fixed EP100 MoE offload."""
...
def run(source_root: Path, finalizer: Path, output: Path, mesh: str = "10x10") -> dict:
    if mesh != "10x10":
    ...
    if output.exists():
        raise ValueError("EP100 compile canary needs a new output root")
    ...
    module_path = Path(__file__).resolve().parents[4] / (
        ...
        "moe_inference_paged_compile_sequence_ep100.py"
    )
    module_name = (
        "llm.frontend.wafer_frontend.passes."
        "moe_inference_paged_compile_sequence_ep100"
    )
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise ValueError("EP100 relinker cannot be loaded")
    ...
    for step, segment in enumerate(sequence.segments):
        ...
        paged = relinker.relink_moe_inference_paged_segment_ep100(source, step)
        artifacts = {}
        ...
        print(f"MoE EP100 compile/finalizer/ProgramIO PASS step={step}", flush=True)
    ...
    result = {
        "schema_version": "moe-ep100-low-hbm-compile-canary-v1",
...
def main() -> None:
    ...
    parser.add_argument("--mesh", choices=("10x10",), default="10x10")
