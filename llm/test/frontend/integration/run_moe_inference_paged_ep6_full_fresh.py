"""Two independent fixed-model six-Die low-HBM MoE offload runs."""
...
_COMPILE = Path(__file__).with_name("run_moe_inference_paged_ep6_compile_canary.py")
_RELINK = _ROOT / "llm/frontend/wafer_frontend/passes/moe_inference_paged_compile_sequence_ep6.py"
_SIDECAR = _ROOT / "llm/frontend/wafer_frontend/passes/moe_inference_paged_runtime_ep6.py"
_SCHEMA = "moe-ep6-low-hbm-two-full-fresh-v1"
...
def _sidecar_module():
    for name, path in (
        ("llm.frontend.wafer_frontend.passes.moe_inference_paged_compile_sequence_ep6", _RELINK),
        ("llm.frontend.wafer_frontend.passes.moe_inference_paged_runtime_ep6", _SIDECAR),
    ):
    ...
    return sys.modules[
        "llm.frontend.wafer_frontend.passes.moe_inference_paged_runtime_ep6"
    ]
...
def _hardware(rows: int, columns: int) -> dict:
    ...
    if (len(physical["hbm_stacks"]) != 6
            or any(item["backend"] != "behavioral"
                   for item in physical["hbm_stacks"])):
        raise ValueError("physical six-Die behavioral HBM layout changed")
...
def _native_observation(fresh: Path, contract: dict, rows: int, columns: int) -> dict:
    ...
    if sidecar != contract or len(sidecar["events"]) != 161:
        raise ValueError("physical EP6 sidecar differs from source-derived contract")
    receipt = _json(compiled / "compile_canary_receipt.json")
    if (receipt["status"] != "compile_finalizer_program_io_only"
            or receipt["mesh"] != f"{rows}x{columns}" or receipt["ep"] != 6):
    ...
    for step, segment in enumerate(receipt["segments"]):
        if (step != segment["step"] or segment["runtime_core_ids"] != [0, 4, 8, 12, 16, 20]
                or segment["program_io_initializations"] != 85
                or segment["program_io_probes"] != 1):
            raise ValueError("EP6 compiled segment/source geometry drifted")
        for role in ("source", "paged"):
            for suffix, name in (("manifest_sha256", "linked.json"),
                ...
                if _sha(path) != segment[role][suffix]:
                    raise ValueError("EP6 production artifact SHA drifted")
        if _sha(compiled / f"segment_{step}.program_io.json") != segment["program_io_sha256"]:
            raise ValueError("EP6 ProgramIO SHA drifted")
    ...
    if len(dma) != 161 or {int(x[0]) for x in dma} != set(range(161)):
        raise ValueError("161 actual source-bound EP6 DMA events absent")
    ...
    for index, event in enumerate(sidecar["events"]):
        ...
        if (int(row[1]) != event["segment_index"]
                ...
                or row[10:] != ("1", "1")):
            raise ValueError(f"physical EP6 DMA gate {index} differs from linked StateABI")
    drain = re.findall(
    ...
    if drain != [("161", "12", "161", "161", "10744", "7488",
                  "7488", "10744", "0", "0", "0", "1")]:
        raise ValueError("physical EP6 external/HBM page traffic did not close")
    ...
    if (len(admission) != 1 or int(admission[0][0]) <= 0
        ...
        raise ValueError("six EP cores did not queue behind fixed two-outstanding link")
    ...
    if (len(memory) != 6 or {item[0] for item in memory} != {0, 4, 8, 12, 16, 20}
            or any(issued <= 0 or issued != completed or lsu or dte
                   for _, issued, completed, lsu, dte in memory)):
        raise ValueError("physical six-core LSU/DTE memory did not close")
    ...
    return {
        ...
        "ep": 6, "mesh": f"{rows}x{columns}", "hbm_capacity_bytes_per_die": 1024,
        "external_capacity_bytes": 8192,
        "dma_events": len(dma), "external_read_bytes": 10744,
        "external_write_bytes": 7488,
...
def run(args: argparse.Namespace) -> None:
    if args.mesh not in ("2x3", "3x2"):
        raise ValueError("only audited six-Die meshes are supported")
    ...
    for index in range(2):
        ...
        if not fresh.exists():
            ...
            if done.returncode:
                raise RuntimeError(f"independent EP6 compile failed Fresh{index}")
        ...
        contract = builder.build_moe_inference_paged_runtime_ep6(
        ...
        if args.resume:
            if (_json(contract_path) != contract
                ...
                raise ValueError("source-derived EP6 sidecar or hardware drifted")
        else:
            ...
            if done.returncode:
                raise RuntimeError(f"physical EP6 native Fresh{index} failed exit={done.returncode}; see {native_path}")
        ...
        if args.resume:
            if _json(evidence_path) != json.loads(json.dumps(item)):
                raise ValueError("physical EP6 Fresh receipt drifted")
        else:
            evidence_path.write_text(json.dumps(item, indent=2, sort_keys=True))
            print(f"MoE EP6 low-HBM PASS full_fresh_{index}", flush=True)
    ...
    if stable(observed[0]) != stable(observed[1]):
        raise ValueError("two independent EP6 full materializations disagree")
    case = {
    ...
    if args.resume:
        if _json(case_path) != json.loads(json.dumps(case)):
            raise ValueError("full EP6 case receipt drifted")
    else:
        case_path.write_text(json.dumps(case, indent=2, sort_keys=True))
    if _binding(args, source_root) != binding:
        raise ValueError("EP6 source/native tool drifted during full Fresh")
    print("MoE EP6 low-HBM VERIFIED two independent full materializations", flush=True)
...
def main() -> None:
    ...
    parser.add_argument("--mesh", choices=("2x3", "3x2"), default="2x3")
