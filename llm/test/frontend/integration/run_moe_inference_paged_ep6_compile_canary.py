"""Compile, relink, finalize and build ProgramIO for fixed EP6 MoE offload."""
...
def run(source_root: Path, finalizer: Path, output: Path, mesh: str = "2x3") -> dict:
    if mesh not in ("2x3", "3x2"):
    ...
    if output.exists():
        raise ValueError("EP6 compile canary needs a new output root")
    ...
    module_path = Path(__file__).resolve().parents[4] / (
        ...
        "moe_inference_paged_compile_sequence_ep6.py"
    )
    module_name = (
        "llm.frontend.wafer_frontend.passes."
        "moe_inference_paged_compile_sequence_ep6"
    )
    spec = importlib.util.spec_from_file_location(module_name, module_path)
    if spec is None or spec.loader is None:
        raise ValueError("EP6 relinker cannot be loaded")
    ...
    for step, segment in enumerate(sequence.segments):
        ...
        paged = relinker.relink_moe_inference_paged_segment_ep6(source, step)
        artifacts = {}
        ...
        print(f"MoE EP6 compile/finalizer/ProgramIO PASS step={step}", flush=True)
    ...
    result = {
        "schema_version": "moe-ep6-low-hbm-compile-canary-v1",
