...
FabricConfig ParseVersionedFabric(const Json &value, uint64_t die_count,
                                  ...
                                  uint64_t external_capacity, uint64_t hbm_capacity,
    ...
    for (const auto &raw : value.at("hbm_capacities")) {
        ...
        if (die >= die_count || String(raw, "tier") != "hbm" ||
            ...
            Number(raw, "capacity_bytes") != hbm_capacity ||
            Number(raw, "alignment_bytes") != 16 ||
            !hbm_refs.emplace(die, String(raw, "id")).second)
            Fail("versioned physical per-Die HBM home changed");
        fabric.hbm_capacities.push_back(
            {String(raw, "id"), die, die * kHome1, hbm_capacity});
    }
...
MoeInferenceMidProgramPager::MoeInferenceMidProgramPager(
    ...
    for (const auto &raw : raw_parameters) {
        ...
        if (span.die_id >= active_die_count_ || span.size_bytes == 0 ||
            span.size_bytes > weight_slot_bytes || span.hbm_address !=
    ...
    for (const auto &raw : raw_kv) {
        ...
        if (span.die_id != 0 || span.size_bytes != 64 ||
            ...
            span.hbm_address < kv_slot_offset ||
            !sources.emplace(span.die_id, span.source_hbm_address).second)
    ...
    for (size_t index = 0; index < 4; ++index)
        if (kv_pages_[index].source_hbm_address != 1344 + 64 * index ||
            ...
            kv_pages_[index].hbm_address != kv_slot_offset + 64 * index)
...
void MoeInferenceMidProgramPager::CompleteSegment(uint64_t segment) {
    const uint64_t expected = segment == 0
        ? (active_die_count_ == 100 ? 615u : active_die_count_ == 9 ? 69u : active_die_count_ == 6 ? 51u : active_die_count_ == 4 ? 39u : 27u)
        : segment == 1
            ? (active_die_count_ == 100 ? 1234u : active_die_count_ == 9 ? 142u : active_die_count_ == 6 ? 106u : active_die_count_ == 4 ? 82u : 58u)
            : expected_events_;
