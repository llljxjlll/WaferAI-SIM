...
int main() {
    ...
    GemmInputDxTimingTile tile{
        ...
        {1024, 128, GemmInputDxDType::FP32},
    };
    ...
    auto bad_tile = tile;
    bad_tile.output.bytes = 64;
    Reject("FP16-sized FP32 dX output", [&] { BuildGemmInputDxTimingWork(source, bad_tile); });
    bad_tile = tile;
    bad_tile.output.dtype = GemmInputDxDType::FP16;
    Reject("FP16 dX substituted", [&] { BuildGemmInputDxTimingWork(source, bad_tile); });
    bad_tile = tile;
    bad_tile.upstream.byte_address = 240;
    Reject("W/dY physical overlap", [&] { BuildGemmInputDxTimingWork(source, bad_tile); });
    bad_tile = tile;
    bad_tile.output.byte_address = 65424;
    ...
    std::cout << "GEMM FP32 input dX NEW-only physical contract: PASS (11 checks)\n";
}
