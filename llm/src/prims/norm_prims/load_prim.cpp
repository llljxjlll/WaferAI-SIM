...
REGISTER_PRIM(Load_prim, PrimId::LEGACY_LOAD);

namespace {
constexpr uint8_t kLoadPrimId = PrimIdValue(PrimId::LEGACY_LOAD);

uint8_t ExpectedPrimId() {
    const int registered = PrimFactory::getInstance().getPrimId("Load_prim");
    if (registered != static_cast<int>(kLoadPrimId))
        throw std::invalid_argument(
            "Load_prim factory ID does not match its fixed PrimId");
    return kLoadPrimId;
}

uint16_t EncodeU16(int value, const char *field) {
    if (value < 0)
        throw std::invalid_argument(std::string("Load_prim ") + field +
                                    " must be non-negative");
    if (static_cast<uint64_t>(value) >
        std::numeric_limits<uint16_t>::max())
        throw std::overflow_error(std::string("Load_prim ") + field +
...
uint8_t EncodeDatatype(DATATYPE value) {
    if (value != INT8 && value != FP16)
        throw std::invalid_argument("Load_prim datatype is invalid");
...
DATATYPE DecodeDatatype(uint64_t value) {
    if (value > static_cast<uint64_t>(FP16))
        throw std::invalid_argument(
            "Load_prim Prim wire datatype is invalid");
...
const sc_bv<128> &ValidateWire(const vector<sc_bv<128>> &segments) {
    if (segments.size() != 1)
        throw std::invalid_argument(
            "Load_prim Prim wire requires exactly one segment");
    const auto &wire = segments.front();
    if (wire.range(7, 0).to_uint64() != ExpectedPrimId())
        throw std::invalid_argument("Load_prim Prim wire ID mismatch");
    if (wire.range(127, 58).or_reduce())
        throw std::invalid_argument(
            "Load_prim Prim wire reserved bits are non-zero");
    return wire;
}
} // namespace
