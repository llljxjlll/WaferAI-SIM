...
gemm_input_dx_timing Specimen() {
    ...
    prim.out_offset = 1024; // FP16 dX[K,M] 64B
...
int RunGemmInputDxTimingPrimSelfTest() {
    ...
    checks.Check(work.tile.weight.dtype == GemmInputDxDType::FP16 &&
                 ...
                 "separate real W[M,N], dY[K,N], FP16 dX[K,M] SRAM spans");
    checks.Check(work.fp16_weight_read_bytes == 256 &&
                 ...
                 "FP16 source and output worker timing work exact");
    ...
    checks.Check(dram == 0 && exu == 1024 && sfu == 0 && vec == 32,
                 "worker charges named FP16 input dX timing");
    ...
    if (decoded) {
        ...
        checks.Check(decoded->work().tile.output.bytes == 64 &&
                     decoded->serialize() == wire,
                     "strict Prim wire roundtrip preserves FP16 dX geometry");
        auto corrupt = wire;
    ...
    bad = Specimen();
    bad.out_offset = 65488;
    checks.Reject("FP16 dX escapes 16-bit SRAM", [&] { bad.serialize(); });
    bad = Specimen();
    ...
    std::cout << "GEMM FP16 input dX strict Prim self-test: "
