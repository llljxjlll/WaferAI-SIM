...
IsaV1SelfTestResult CheckIsaV1OpcodeManifest() {
    ...
    Check(result, CountCategory(OpcodeCategory::COMPUTE) == 44,
          "compute range contains 44 assigned opcodes");
    ...
    constexpr std::array<uint8_t, 7> kReservedEncoding{{
        0x00, 0x2d, 0x47, 0x8a, 0xc7, 0xf0, 0xff,
    }};
...
IsaV1SelfTestResult CheckIsaV1PrimManifest() {
    ...
    Check(result, compute == 51 && communication == 8 && memory == 14 &&
...
int RunIsaV1SelfTest() {
    ...
    const int gemm_input_dx_wire_failures =
        RunGemmInputDxTimingPrimSelfTest();
