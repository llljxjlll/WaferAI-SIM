"""Relink the fixed six-die full MoE inference source into real low-HBM pages.
...
HBM_HOME_BASE = tuple(die << 30 for die in range(6))
...
def _validate_source(source: LinkedProgramManifest, step: int) -> tuple[StateABI, ...]:
    """Require the actual six-core full-model source and its physical ABI."""
    if step not in (0, 1, 2):
        raise SchemaError("requires true Prefill+2Decode segment", path="step")
    source.validate("moe_paged_ep6.source")
    expected_records = ((207, 52, 52, 52, 30, 30) if step == 0
                        else (199, 52, 30, 30, 30, 30))
    if (
        ...
        or tuple(len(stream.records) for stream in source.core_streams)
            != expected_records
        or len(source.state_operand_bindings) != (51 if step == 0 else 55)
    ):
        raise SchemaError("two-layer EP6 full-model linked source shape changed", path="source")
    ...
    if (len(abis) != 39 or len(shared0) != 13
            or sum(abi.size_bytes for abi in shared0) != 648
            or len(kv) != 4):
        raise SchemaError("EP6 shared/router/expert/KV StateABI inventory changed",
    ...
    for die in range(6):
        ...
        if (len(experts) != 2 or any(abi.size_bytes != 192 for abi in experts)
                or len(gates) != 2 or any(abi.size_bytes != 48 for abi in gates)):
            raise SchemaError("EP6 per-Die expert/router pages changed",
                              path="source.state_abi")
        ...
        if (tuple(sorted(abi.address for abi in experts)) != expected_expert
            ...
            raise SchemaError("EP6 expert source HBM home/hole anchoring changed",
                              path="source.state_abi")
    page = 32 + 16 * step
    if any(abi.die_id != 0
...
def relink_moe_inference_paged_segment_ep6(
    source: LinkedProgramManifest, step: int,
) -> LinkedProgramManifest:
    """Retain all six production EP cores in 1024B per-Die HBM."""
...
__all__ = ["relink_moe_inference_paged_segment_ep6", "unique_state_abis"]
