"""Source-bound actual-LSU external pages for the fixed full six-expert EP6 MoE model."""
...
from .moe_inference_paged_compile_sequence_ep6 import (
...
PARAMETER_EXTERNAL_RESERVED_BYTES = 6240
KV_EXTERNAL_BASE = 6240
EXTERNAL_CAPACITY_BYTES = 8192
PHYSICAL_PARAMETER_BYTES = 3432
...
def _external_parameter_allocations(offload):
    ...
    for allocation in plan.allocations:
        ...
        if (state.logical_rank not in range(6)
                or request.size_bytes not in (384, 648)
                or (state.logical_rank, role) in by_rank_role):
            raise SchemaError("EP6 external rank/role allocation changed",
                              path="offload.memory_plan")
        by_rank_role[state.logical_rank, role] = (
            allocation.address, request.size_bytes, allocation.id
        )
    if set(by_rank_role) != {(die, role) for die in range(6)
                            for role in ("expert", "shared")}:
        raise SchemaError("twelve true P3 EP6 external allocations missing",
                          path="offload.memory_plan")
...
def _spans(source: LinkedProgramManifest, allocations):
    ...
    if len(abis) != 39:
        raise SchemaError("39 true EP6 source StateABI pages required",
    ...
    for die in range(6):
        ...
        if (len(experts) != 2 or sum(abi.size_bytes for abi in experts) != 384
                ...
                or sum(abi.size_bytes for abi in shared) != (648 if die == 0 else 96)):
            raise SchemaError("actual EP6 source physical weight inventory changed",
                              path="source.state_abi")
    ...
    if (len(spans) != 35
        ...
        raise SchemaError("35 true EP6 physical parameter pages missing",
                          path="source.state_abi")
...
def build_moe_inference_paged_runtime_ep6(
    *, resident, offload,
        ...
        {item.location_ref: (item.base_address, item.capacity_bytes)
         for item in fabric.hbm_capacities} !=
            {f"die:{die}": (HBM_HOME_BASE[die], HBM_CAPACITY_BYTES)
             for die in range(6)} or
        len(fabric.links) != 1 or len(fabric.connections) != 6
    ):
        raise SchemaError("official one-link/six-die low-HBM fabric changed", path="fabric")
    ...
    for die in range(6):
        ...
        if end != (464, 464, 464, 456, 464, 456)[die]:
    ...
    for segment, (source, paged) in enumerate(zip(source_manifests, paged_manifests)):
        ...
        if len(source.core_streams) != 6 or len(paged.core_streams) != 6:
            raise SchemaError("six real MoE cores required", path="manifests")
        ...
        for old_core, new_core in zip(source.core_streams, paged.core_streams):
            ...
            for index, (old_ref, new_ref) in enumerate(zip(old_core.records,
                ...
                counts[kind] += 1
        if counts != {
            "weight_restore_before_load": 35,
            "expert_writeback_after_store": 12,
            "kv_restore_before_load": 0 if segment == 0 else 4,
            "kv_writeback_after_store": 4,
        }:
            raise SchemaError("MoE 35 weights/12 expert roundtrip/4KV exact linked gates changed", path="manifests")
        per_core_counts.append(counts)
    if len(events) != 161:
        raise SchemaError("all 161 physical MoE LSU gates required", path="events")
    ...
    if (reads, writes) != (10744, 7488):
        raise SchemaError("independent true StateABI transfer byte oracle changed", path="events")
    # Seed exactly the 35 physical parameter pages. Ranks 1..5 each have
    ...
    key: dict[str, object] = {
        "schema_version": "wafer_frontend.moe_inference_paged_runtime/v3alpha1",
        ...
        "source_external_parameter_allocation_refs": [
            external_allocations[die, role][2] for die in range(6)
            for role in ("expert", "shared")
        ],
        ...
        "active_die_ids": list(range(6)),
        "expected_dma_events": 161,
        "expected_external_read_bytes": 10744,
        "expected_external_write_bytes": 7488,
...
__all__ = ["build_moe_inference_paged_runtime_ep6"]
