"""Layer1 EP1 expert derivative from the actual 0x28 dExpert tensor.

Physical lowering must recompute gate/up and SwiGLU from the saved forward
activation and three real weights; this source never invents an activation tape.
"""
...
def append_moe_full_train_expert_backward_ir0(source: IR0) -> IR0:
    ...
    if (forward is None or forward.kind is not OpKind.MOE_EXPERT_FORWARD
            ...
            or forward.workload.expert_count != 1
            or forward.workload.expert != 0
            or forward.workload.source_route_trace_digest !=
               combine.workload.source_route_trace_digest):
        raise SchemaError("same-layer expert forward/0x28 return tape absent",
                          path="source.nodes")
    m, h, i = (forward.workload.owned_token_count,
               forward.workload.hidden_size,
               forward.workload.intermediate_size)
    if (values[combine.outputs[1]].shape != (m, h)
            or values[combine.outputs[1]].dtype is not DType.FP16):
        raise SchemaError("native dExpert shape differs from expert return",
                          path="source.nodes")
    ...
    backward = LogicalNode(
        ...
        (*forward.inputs, combine.outputs[1]), output_ids,
