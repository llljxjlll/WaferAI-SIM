"""Source-bound actual-LSU external pages for the fixed full hundred-expert EP100 MoE model."""
...
def build_moe_inference_paged_runtime_ep100(
    *, resident, offload,
    ...
    if (
        ...
        len(fabric.hbm_capacities) != 100 or
        {item.location_ref: (item.base_address, item.capacity_bytes)
         for item in fabric.hbm_capacities} !=
            {f"die:{die}": (HBM_HOME_BASE[die], HBM_CAPACITY_BYTES)
             for die in range(100)} or
        len(fabric.links) != 1 or len(fabric.connections) != 100
    ):
        raise SchemaError("official one-link/hundred-die low-HBM fabric changed", path="fabric")
    ...
    key: dict[str, object] = {
        "schema_version": "wafer_frontend.moe_inference_paged_runtime/v5alpha1",
...
__all__ = ["build_moe_inference_paged_runtime_ep100"]
