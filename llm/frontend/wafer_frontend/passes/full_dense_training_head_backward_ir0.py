...
def append_dense_training_head_backward_source(source: IR0) -> IR0:
    ...
    if (
        source.producer_pass != "dense_training_ce_backward_source"
        or source.job is not JobKind.TRAIN
        or len(source.instances) != 1
        or source.instances[0].parallel.tp != 1
        or len(source.nodes) < 3
    ):
        raise SchemaError("requires complete TP1 Dense CE backward source", path="source")
    ...
    result = IR0.create(
        producer_pass="dense_training_head_backward_source", job=source.job,
    ...
    result.validate("dense_training_head_backward_source")
    return result


__all__ = ["append_dense_training_head_backward_source"]
