"""Relink the fixed hundred-die full MoE inference source into real low-HBM pages.
...
HBM_HOME_BASE = tuple(die << 30 for die in range(100))
HBM_CAPACITY_BYTES = 2048
P3_WORKSPACE_END_BYTES = 464
WEIGHT_SLOT_OFFSET = 512
WEIGHT_SLOT_BYTES = 800
KV_SLOT_OFFSET = 1344
KV_SLOT_BYTES = 64
HIGHEST_PAGED_END_BYTES = 1600
KV_SOURCE_BASE = 1344
KV_SOURCE_STRIDE = 64
...
def _validate_source(source: LinkedProgramManifest, step: int) -> tuple[StateABI, ...]:
    """Require the actual hundred-core full-model source and its physical ABI."""
    if step not in (0, 1, 2):
        raise SchemaError("requires true Prefill+2Decode segment", path="step")
    source.validate("moe_paged_ep100.source")
    expected_records = ((207, 52, 52, 52) + (30,) * 96 if step == 0
                        else (199, 52) + (30,) * 98)
    if (
        ...
        or tuple(stream.runtime_core_id for stream in source.core_streams)
            != tuple(die * 4 for die in range(100))
        or tuple(len(stream.records) for stream in source.core_streams)
            != expected_records
        or len(source.state_operand_bindings) != (615 if step == 0 else 619)
    ):
        raise SchemaError("two-layer EP100 full-model linked source shape changed", path="source")
    ...
    if (len(abis) != 415 or len(shared0) != 13
            or sum(abi.size_bytes for abi in shared0) != 2152
            or len(kv) != 4):
        raise SchemaError("EP100 shared/router/expert/KV StateABI inventory changed",
    ...
    for die in range(100):
        ...
        if (len(experts) != 2 or any(abi.size_bytes != 192 for abi in experts)
                or len(gates) != 2 or any(abi.size_bytes != 800 for abi in gates)):
            raise SchemaError("EP100 per-Die expert/router pages changed",
                              path="source.state_abi")
        ...
        if (tuple(sorted(abi.address for abi in experts)) != expected_expert
            ...
            raise SchemaError("EP100 expert source HBM home/hole anchoring changed",
                              path="source.state_abi")
    page = 32 + 16 * step
    if any(abi.die_id != 0
...
def relink_moe_inference_paged_segment_ep100(
    source: LinkedProgramManifest, step: int,
) -> LinkedProgramManifest:
    """Retain all hundred production EP cores in 1024B per-Die HBM."""
...
__all__ = ["relink_moe_inference_paged_segment_ep100", "unique_state_abis"]
