"""Source-bound actual-LSU external pages for the fixed full nine-expert EP9 MoE model."""
...
from .moe_inference_paged_compile_sequence_ep9 import (
...
PARAMETER_EXTERNAL_RESERVED_BYTES = 9792
KV_EXTERNAL_BASE = 9792
EXTERNAL_CAPACITY_BYTES = 12288
PHYSICAL_PARAMETER_BYTES = 5304
...
def _external_parameter_allocations(offload):
    ...
    for allocation in plan.allocations:
        ...
        if (state.logical_rank not in range(9)
                or request.size_bytes not in (384, 696)
                or (state.logical_rank, role) in by_rank_role):
            raise SchemaError("EP9 external rank/role allocation changed",
                              path="offload.memory_plan")
        by_rank_role[state.logical_rank, role] = (
            allocation.address, request.size_bytes, allocation.id
        )
    if set(by_rank_role) != {(die, role) for die in range(9)
                            for role in ("expert", "shared")}:
        raise SchemaError("twelve true P3 EP9 external allocations missing",
                          path="offload.memory_plan")
...
def _spans(source: LinkedProgramManifest, allocations):
    ...
    if len(abis) != 51:
        raise SchemaError("51 true EP9 source StateABI pages required",
    ...
    for die in range(9):
        ...
        if (len(experts) != 2 or sum(abi.size_bytes for abi in experts) != 384
                ...
                or sum(abi.size_bytes for abi in shared) != (696 if die == 0 else 144)):
            raise SchemaError("actual EP9 source physical weight inventory changed",
                              path="source.state_abi")
    ...
    if (len(spans) != 47
        ...
        raise SchemaError("47 true EP9 physical parameter pages missing",
                          path="source.state_abi")
...
def build_moe_inference_paged_runtime_ep9(
    *, resident, offload,
    ...
    if (
        ...
        len(fabric.hbm_capacities) != 9 or
        {item.location_ref: (item.base_address, item.capacity_bytes)
         for item in fabric.hbm_capacities} !=
            {f"die:{die}": (HBM_HOME_BASE[die], HBM_CAPACITY_BYTES)
             for die in range(9)} or
        len(fabric.links) != 1 or len(fabric.connections) != 9
    ):
        raise SchemaError("official one-link/nine-die low-HBM fabric changed", path="fabric")
    ...
    for die in range(9):
        ...
        if end != (464, 464, 464, 456, 464, 456, 464, 464, 464)[die]:
    ...
    for segment, (source, paged) in enumerate(zip(source_manifests, paged_manifests)):
        ...
        if len(source.core_streams) != 9 or len(paged.core_streams) != 9:
            raise SchemaError("nine real MoE cores required", path="manifests")
        ...
        for old_core, new_core in zip(source.core_streams, paged.core_streams):
            ...
            for index, (old_ref, new_ref) in enumerate(zip(old_core.records,
                ...
                counts[kind] += 1
        if counts != {
            "weight_restore_before_load": 47,
            "expert_writeback_after_store": 18,
            "kv_restore_before_load": 0 if segment == 0 else 4,
            "kv_writeback_after_store": 4,
        }:
            raise SchemaError("MoE 47 weights/18 expert roundtrip/4KV exact linked gates changed", path="manifests")
        per_core_counts.append(counts)
    if len(events) != 215:
        raise SchemaError("all 215 physical MoE LSU gates required", path="events")
    ...
    if (reads, writes) != (16360, 10944):
        raise SchemaError("independent true StateABI transfer byte oracle changed", path="events")
    # Seed exactly the 47 physical parameter pages. Ranks 1..8 each have
    ...
    key: dict[str, object] = {
        "schema_version": "wafer_frontend.moe_inference_paged_runtime/v4alpha1",
        ...
        "source_external_parameter_allocation_refs": [
            external_allocations[die, role][2] for die in range(9)
            for role in ("expert", "shared")
        ],
        ...
        "active_die_ids": list(range(9)),
        "expected_dma_events": 215,
        "expected_external_read_bytes": 16360,
        "expected_external_write_bytes": 10944,
...
__all__ = ["build_moe_inference_paged_runtime_ep9"]
