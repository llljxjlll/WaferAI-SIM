"""Relink the fixed two-die full MoE inference source into real low-HBM pages.
...
def _validate_source(source: LinkedProgramManifest, step: int) -> tuple[StateABI, ...]:
    ...
    if (
        ...
        raise SchemaError("two-layer EP2 full-model linked source shape changed", path="source")
    abis = unique_state_abis(source)
    per_die = {die: tuple(abi for abi in abis if abi.die_id == die) for die in (0, 1)}
    die0_shared = tuple(abi for abi in per_die[0] if abi.kind is StateKind.PARAMETER)
    die0_expert = tuple(abi for abi in per_die[0] if abi.kind is StateKind.TRAINABLE_PARAMETER)
    die1_gate = tuple(abi for abi in per_die[1] if abi.kind is StateKind.PARAMETER)
    die1_expert = tuple(abi for abi in per_die[1] if abi.kind is StateKind.TRAINABLE_PARAMETER)
    kv = tuple(sorted((abi for abi in abis if abi.kind in
                       (StateKind.KV_KEY, StateKind.KV_VALUE)), key=lambda abi: abi.address))
    if (
        len(abis) != 23 or len(die0_shared) != 13 or
        sum(abi.size_bytes for abi in die0_shared) != 584 or
        len(die0_expert) != 2 or any(abi.size_bytes != 192 for abi in die0_expert)
        or len(die1_gate) != 2 or any(abi.size_bytes != 16 for abi in die1_gate)
        or len(die1_expert) != 2 or any(abi.size_bytes != 192 for abi in die1_expert)
        or len(kv) != 4
    ):
        raise SchemaError("true shared/router/expert/KV StateABI inventory changed", path="source.state_abi")
    for die, experts, gates in ((0, die0_expert, tuple(abi for abi in die0_shared
                                                        if abi.address >= 1 << 24)),
                                (1, die1_expert, die1_gate)):
        expected_expert = tuple(HBM_HOME_BASE[die] + (1 << 24) + layer * (1 << 20)
                                for layer in (0, 1))
        if tuple(sorted(abi.address for abi in experts)) != expected_expert or tuple(
            sorted(abi.address for abi in gates)
        ) != tuple(address + 4096 for address in expected_expert):
            raise SchemaError("expert source HBM home/hole anchoring changed", path="source.state_abi")
    page = 32 + 16 * step
    if any(
        abi.die_id != 0 or abi.address != KV_SOURCE_BASE + index * KV_SOURCE_STRIDE
        or abi.size_bytes != page
        for index, abi in enumerate(kv)
    ):
        raise SchemaError("four physical KV pages/version lengths changed", path="source.state_abi")
    if any(abi.size_bytes > WEIGHT_SLOT_BYTES for abi in (*die0_shared, *die0_expert,
                                                           *die1_gate, *die1_expert)):
        raise SchemaError("one genuine parameter ABI does not fit the timed slot", path="source.state_abi")
    if not (P3_WORKSPACE_END_BYTES <= WEIGHT_SLOT_OFFSET and
            WEIGHT_SLOT_OFFSET + WEIGHT_SLOT_BYTES <= KV_SLOT_OFFSET and
            HIGHEST_PAGED_END_BYTES <= HBM_CAPACITY_BYTES):
        raise SchemaError("timed slots collide with P3 workspace or low HBM", path="slots")
    return abis


def relink_moe_inference_paged_segment(
    source: LinkedProgramManifest, step: int,
) -> LinkedProgramManifest:
    """Retain full production MoE inference records in 1024B per-die HBM."""
...
__all__ = ["relink_moe_inference_paged_segment", "unique_state_abis"]
