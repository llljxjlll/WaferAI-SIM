"""Source-bound EP100 pages on a larger physical mesh with signed idle Die."""
...
from .moe_inference_paged_compile_sequence_ep100 import (
...
PARAMETER_EXTERNAL_RESERVED_BYTES = 254392
KV_EXTERNAL_BASE = 254392
EXTERNAL_CAPACITY_BYTES = 262144
PHYSICAL_PARAMETER_BYTES = 198952
KV_PHYSICAL_BYTES = 256
_EXPECTED_WORKSPACE_SHA256 = "7a3dbf71b5a05fb20dfc03369d9a74a53b33e59911b83644eb34dd3b87cb3a4b"
...
def _external_parameter_allocations(offload):
    ...
    for allocation in plan.allocations:
        ...
        if (state.logical_rank not in range(100)
                or request.size_bytes not in (384, 2152)
                or (state.logical_rank, role) in by_rank_role):
            raise SchemaError("EP100 external rank/role allocation changed",
                              path="offload.memory_plan")
        by_rank_role[state.logical_rank, role] = (
            allocation.address, request.size_bytes, allocation.id
        )
    if set(by_rank_role) != {(die, role) for die in range(100)
                            for role in ("expert", "shared")}:
        raise SchemaError("two hundred true P3 EP100 external allocations missing",
                          path="offload.memory_plan")
...
def _spans(source: LinkedProgramManifest, allocations):
    ...
    if len(abis) != 415:
        raise SchemaError("415 true EP100 source StateABI pages required",
    ...
    for die in range(100):
        ...
        if (len(experts) != 2 or sum(abi.size_bytes for abi in experts) != 384
                ...
                or sum(abi.size_bytes for abi in shared) != (2152 if die == 0 else 1600)):
            raise SchemaError("actual EP100 source physical weight inventory changed",
                              path="source.state_abi")
    ...
    if (len(spans) != 411
        ...
        raise SchemaError("411 true EP100 physical parameter pages missing",
                          path="source.state_abi")
...
def build_moe_inference_paged_runtime_ep100_sparse_v6(
    *, resident, offload,
    source_manifests: tuple[LinkedProgramManifest, ...],
    paged_manifests: tuple[LinkedProgramManifest, ...],
    fabric: ExternalMemoryFabric,
) -> dict[str, object]:
    """Sign every actual hundred-core weight, expert retention and KV gate."""
    ...
    fabric.validate()
    rows, columns = resident.request.mesh.rows, resident.request.mesh.columns
    physical_die_count = rows * columns
    active_die_ids = tuple(range(100))
    if ((rows, columns) not in ((11, 11), (12, 12))
            or resident.request.parallel.ep != 100
            or resident.request.parallel.active_die_ids != active_die_ids
            or offload.request.parallel.active_die_ids != active_die_ids):
        raise SchemaError("v6 requires fixed100 identity-prefix sparse placement", path="request")
    if (
        len(fabric.external_capacities) != 1 or
        fabric.external_capacities[0].capacity_bytes != EXTERNAL_CAPACITY_BYTES or
        len(fabric.hbm_capacities) != physical_die_count or
        {item.location_ref: (item.base_address, item.capacity_bytes)
         for item in fabric.hbm_capacities} !=
            {f"die:{die}": (die << 30, HBM_CAPACITY_BYTES)
             for die in range(physical_die_count)} or
        len(fabric.links) != 1 or len(fabric.connections) != physical_die_count
    ):
        raise SchemaError("v6 one-link/physical sparse low-HBM fabric changed", path="fabric")
    ...
    for die in range(100):
        ...
        workspace_ends.append(end)
    if hashlib.sha256(json.dumps(workspace_ends, separators=(",", ":")).encode()).hexdigest() != _EXPECTED_WORKSPACE_SHA256:
    ...
    for segment, (source, paged) in enumerate(zip(source_manifests, paged_manifests)):
        ...
        if len(source.core_streams) != 100 or len(paged.core_streams) != 100:
            raise SchemaError("hundred real MoE cores required", path="manifests")
        ...
        for old_core, new_core in zip(source.core_streams, paged.core_streams):
            ...
            for index, (old_ref, new_ref) in enumerate(zip(old_core.records,
                ...
                counts[kind] += 1
        if counts != {
            "weight_restore_before_load": 411,
            "expert_writeback_after_store": 200,
            "kv_restore_before_load": 0 if segment == 0 else 4,
            "kv_writeback_after_store": 4,
        }:
            raise SchemaError("MoE 411 weights/200 expert roundtrip/4KV exact linked gates changed", path="manifests")
        per_core_counts.append(counts)
    if len(events) != 1853:
        raise SchemaError("all 1853 physical MoE LSU gates required", path="events")
    ...
    if (reads, writes) != (597304, 115776):
        raise SchemaError("independent true StateABI transfer byte oracle changed", path="events")
    # Seed exactly the 411 physical parameter pages. Ranks 1..99 each have
    ...
    key: dict[str, object] = {
        "schema_version": "wafer_frontend.moe_inference_paged_runtime/v6alpha1",
        ...
        "source_external_parameter_allocation_refs": [
            external_allocations[die, role][2] for die in range(100)
            for role in ("expert", "shared")
        ],
        ...
        "active_die_ids": list(range(100)),
        "physical_die_count": physical_die_count,
        "idle_die_ids": list(range(100, physical_die_count)),
        "expected_dma_events": 1853,
        "expected_external_read_bytes": 597304,
        "expected_external_write_bytes": 115776,
...
__all__ = ["build_moe_inference_paged_runtime_ep100_sparse_v6"]
