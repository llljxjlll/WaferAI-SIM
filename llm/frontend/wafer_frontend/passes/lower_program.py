...
def _lower_fragments(
    ...
    for action_index, action in enumerate(actions):
        ...
        if action.op_kind is OpKind.MOE_ROUTE_FREEZE:
        ...
        elif action.op_kind is OpKind.MOE_COMBINE_BACKWARD:
            from ..lowering.moe_full_train_combine_backward import (
                lower_moe_combine_backward,
            )
            fragment = lower_moe_combine_backward(action, context)
            ...
            schedule = next((item for item in context.schedule_set.schedules
                             if item.id == action.source.schedule_id), None)
            if schedule is None:
                raise SchemaError("expert action lacks its physical schedule",
                                  path=f"source.global_dag.actions[{action_index}]")
