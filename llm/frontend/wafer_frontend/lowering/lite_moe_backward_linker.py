...
def _runtime_cores(source: LiteMoeBackwardLoweredProgram) -> dict[int, tuple[LogicalCoreRef, int, str]]:
    logical_cores = {
        stream.logical_core
        for fragment in source.fragments
        for stream in fragment.core_streams
    }
    if {core.die_id for core in logical_cores} != {0, 1} or len(logical_cores) != 2:
...
def _action_contract(source: LiteMoeBackwardLoweredProgram):
    ...
    for unit in overlay.remote_grad_dtes:
        add(f"{unit.id}.send", unit.source_die_id, (1, unit.token_index, 0))
        add(f"{unit.id}.recv", unit.destination_die_id, (1, unit.token_index, 1))
        add(f"{unit.id}.wait", unit.destination_die_id, (1, unit.token_index, 2))
    ...
    if len(actions) != 32:
        raise SchemaError("backward action contract must contain 32 claims", path="overlay")
    return actions
...
def _runtime_definitions(
    ...
    unit_by_action = {
        action_id: unit
        for unit in source.overlay.remote_grad_dtes
        for action_id in (f"{unit.id}.send", f"{unit.id}.recv", f"{unit.id}.wait")
    }
    definitions = []
    for symbol_id in sorted(declarations):
        ...
        source_core = actions[f"{unit.id}.send"][0]
        destination_core = actions[f"{unit.id}.recv"][0]
        if symbol.kind is RuntimeSymbolKind.DTE_FSM:
            ...
            if occurrence_actions == {f"{unit.id}.send"}:
                peer = destination_core
            elif occurrence_actions == {f"{unit.id}.recv"}:
                peer = source_core
            else:
...
def _select_buffer(
    ...
    if record.opcode is RecordOpcode.SRAM_ALLOC_AT:
        ...
        wgrad = next((unit for unit in source.overlay.token_wgrads if unit.id == record.source_global_action_id), None)
        sgd = next((unit for unit in source.overlay.sgd_stores if unit.id == record.source_global_action_id), None)
        if wgrad is not None:
            expected_value = {
                SemanticOperandId.SRAM_BIND_INPUT_0: wgrad.saved_activation_ref,
                SemanticOperandId.SRAM_BIND_INPUT_1: (
                    f"{wgrad.upstream_grad_ref}.received"
                    if any(item.token_index == wgrad.token_index for item in source.overlay.remote_grad_dtes)
                    else wgrad.upstream_grad_ref
                ),
                SemanticOperandId.SRAM_BIND_OUTPUT: wgrad.contribution_ref,
            }.get(operand_id)
        elif sgd is not None:
            state = next(item for item in source.overlay.trainable_down_states if item.expert_index == sgd.expert_index)
            expected_value = {
...
def link_lite_moe_backward_manifest(
    source: LiteMoeBackwardLoweredProgram,
) -> LinkedProgramManifest:
    """Link the exact 28-leaf backward quotient into one executable manifest."""
    ...
    for fragment in fragments:
        ...
        if fragment.source_global_dag_id != source.overlay.id:
            ...
            for relocation in stream.address_relocations:
                ...
                if relocation.operand_id is SemanticOperandId.HBM_ADDRESS:
                    ...
                    if (
                        ...
                        units = tuple(
                            unit
                            for unit in source.overlay.expert_reduces
    ...
    region_specs = {
        (die.id, core.local_core_id): next(
            profile for profile in source.n4.graph.fabric.sram_profiles
    ...
    for die_id in (0, 1):
        ...
        die = next(item for item in source.n4.graph.fabric.dies if item.id == die_id)
        ...
        symbol = RuntimeSymbol(
            stable_artifact_id(
                "start_tag",
                {
                    "source_global_dag_id": source.overlay.id,
    ...
    artifacts = [
        ...
        (ManifestInputKind.GLOBAL_ACTION_DAG, source.overlay),
    ...
    active_cores = tuple(cores[die][0] for die in (0, 1))
    ...
    manifest = LinkedProgramManifest.create(
        producer_pass="lite_moe_backward_manifest_linker",
        capabilities=0,
...
__all__ = ["link_lite_moe_backward_manifest"]
