...
from ..passes.lite_moe_backward import validate_lite_moe_backward_overlay
...
_PRODUCER = "lite_moe_backward_lowering"
...
def _state_abi(item, source: LiteMoeN4IR1) -> StateABI:
    home = next(
...
def lower_lite_moe_backward(
    ...
    """Lower the exact typed WGRAD-only overlay into 28 canonical leaves."""
    ...
    for unit in overlay.token_wgrads:
        ...
        saved[unit.token_index] = reserve(
            unit.home_die_id, 64, unit.saved_activation_ref, (1, 32),
    ...
    remote = {item.token_index: item for item in overlay.remote_grad_dtes}
    for unit in overlay.token_wgrads:
        source_die = remote[unit.token_index].source_die_id if unit.token_index in remote else unit.home_die_id
        source = reserve(
            source_die, 32, unit.upstream_grad_ref, (1, 16),
        ...
        upstream_home[unit.token_index] = (
            reserve(
                unit.home_die_id, 32, f"{unit.upstream_grad_ref}.received",
    ...
    # Four exact 32-byte remote-gradient endpoint pairs.
    for unit in overlay.remote_grad_dtes:
        send_id, recv_id, wait_id = (
            f"{unit.id}.send", f"{unit.id}.recv", f"{unit.id}.wait"
        )
        source = upstream_source[unit.token_index]
        destination = upstream_home[unit.token_index]
        fsm = _runtime_symbol(RuntimeSymbolKind.DTE_FSM, unit.forward_flow_ref,
                              ("lite_moe_backward", unit.id, "fsm"))
        token = _runtime_symbol(RuntimeSymbolKind.DTE_TOKEN, unit.id,
                                ("lite_moe_backward", unit.id, "token"))
        send_peer = _runtime_symbol(RuntimeSymbolKind.RUNTIME_CORE, unit.pair_route_ref,
                                    ("lite_moe_backward", unit.id, "send_peer"))
        recv_peer = _runtime_symbol(RuntimeSymbolKind.RUNTIME_CORE, unit.pair_route_ref,
    ...
    if len(result) != 28:
        raise SchemaError("backward lowering must emit exactly 28 leaves", path="fragments")
    return result


def validate_lite_moe_backward_fragments(
