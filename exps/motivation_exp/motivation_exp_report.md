...
* **无拥塞重叠（片上 y 方向 AG）**：跨 die 流量与片上任务**完美重叠**，
  实测相对理想值 `max(W_片上, W_跨die)` 的偏差只有 **-0.0% ~ +0.0%**。
* **拥塞重叠（片上 x 方向 AG）**：同样的两份工作放在一起跑，却比理想值多
  **+8.2% ~ +48.8%**。多出来的部分**全部**来自 NoC 物理链路争用 ——
...
* x 方向 AG ∩ 过境流量 = `0->1, 1->2, 2->3, 12->13, 13->14, 14->15` —— 正好是第 0 行和第 3 行的 3 条东向链路各一组；
* y 方向 AG ∩ 过境流量 = `（空集）`。

仿真器侧的记账也确认两者等价：片上任务单独跑时 die1 的 NoC 发送数
x 方向 6368、y 方向 6368（完全相同），跨 die 单独跑时
die1 的 NoC 发送数 9216（= 2 条流 × 包数 × 3 跳，与 AG 方向无关）。
...
* 声明了 die1 那 16 个计算核的场景（`onchip_*` / `overlap_*`）：前导 **366 ns**；
* 只有 4 个核的纯跨 die 场景（`xdie`）：前导 **142 ns**。

如果直接拿两次运行的**总时间**相减，这 366 − 142 ns 的
...
| inter/intra 流量比 | 单条过境流 | 片上 W_on | 跨 die W_xd | 无重叠基线 | 拥塞重叠(x) | 无拥塞重叠(y) | Norm.(x) | Norm.(y) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 0.25x | 384 pkt | 6775 ns | 2650 ns | 9425 ns | 7330 ns | 6662 ns | 0.778 | 0.707 |
| 0.5x | 768 pkt | 6775 ns | 4974 ns | 11749 ns | 9654 ns | 6662 ns | 0.822 | 0.567 |
| 1x | 1536 pkt | 6775 ns | 9598 ns | 16373 ns | 14278 ns | 9598 ns | 0.872 | 0.586 |
| 2x | 3072 pkt | 6775 ns | 18932 ns | 25707 ns | 23612 ns | 18932 ns | 0.919 | 0.736 |


片上任务单独跑的总时间：x 方向 7141 ns，y 方向 7029 ns
...
| 流量比 | 无拥塞(y) 理想 | 实测(y) | 相对理想 | 拥塞(x) 理想 | 实测(x) | 相对理想 |
|---|---:|---:|---:|---:|---:|---:|
| 0.25x | 6663 ns | 6662 ns | -0.0% | 6775 ns | 7330 ns | +8.2% |
| 0.5x | 6663 ns | 6662 ns | -0.0% | 6775 ns | 9654 ns | +42.5% |
| 1x | 9598 ns | 9598 ns | +0.0% | 9598 ns | 14278 ns | +48.8% |
| 2x | 18932 ns | 18932 ns | +0.0% | 18932 ns | 23612 ns | +24.7% |
...
| 运行 | 组 0 (y=0 / x=0) | 组 1 | 组 2 | 组 3 (y=3 / x=3) | 过境流完成 |
|---|---:|---:|---:|---:|---:|
| 片上 x 方向 AG（无跨 die 流量） | 3499 | 3517 | 3535 | 3553 | — |
| 片上 y 方向 AG（无跨 die 流量） | 3499 | 3499 | 3499 | 3499 | — |
| 仅跨 die 过境流量，0.25x | — | — | — | — | 1361/1383 |
| 拥塞重叠 x，0.25x | 3809 | 3517 | 3535 | 3831 | 1473/1495 |
| 无拥塞重叠 y，0.25x | 3499 | 3499 | 3499 | 3499 | 1473/1495 |
| 仅跨 die 过境流量，0.5x | — | — | — | — | 2523/2545 |
| 拥塞重叠 x，0.5x | 4971 | 3517 | 3535 | 4993 | 2635/2657 |
| 无拥塞重叠 y，0.5x | 3499 | 3499 | 3499 | 3499 | 2635/2657 |
| 仅跨 die 过境流量，1x | — | — | — | — | 4835/4857 |
| 拥塞重叠 x，1x | 7283 | 3517 | 3535 | 7305 | 4947/4969 |
| 无拥塞重叠 y，1x | 3499 | 3499 | 3499 | 3499 | 4947/4969 |
| 仅跨 die 过境流量，2x | — | — | — | — | 9502/9524 |
| 拥塞重叠 x，2x | 11950 | 3517 | 3535 | 11972 | 9614/9636 |
| 无拥塞重叠 y，2x | 3499 | 3499 | 3499 | 3499 | 9614/9636 |


读法一（延迟落在谁身上）：**过境流量本身几乎没被拖慢**。对比「仅跨 die」行
和两个重叠行，过境流到达 die2 的 cycle 只推后了固定的 112 cycle，而且
**x 与 y 两个重叠场景推后的量完全一样**（脚本会强制校验这一点，不一致就直接报错）
—— y 场景一条链路都不共享，所以这 112 cycle 只是「workload 里多了 16 个核，host 配置相变长」的启动偏移，不是
